...
Nella repository sono inoltre presenti gli output editoriali generati automaticamente (PDF, EPUB, DOCX) ottimizzati per la stampa e gli e-reader, e i file di metadati strutturati (ONIX e Schema.org) pronti per l'indicizzazione e la distribuzione commerciale.
...
3. **Compilazione:** Pandoc (con XeLaTeX o CSS personalizzati) compila i file di output (PDF, EPUB, DOCX e Web-Book).
...
[input.md] + [metadati.yaml]
  ...
  ( main.py ) ----------------> [ Metadati: ONIX & Schema.org ]
   ...
   [ Pandoc ]
       |
       +---> Cartella 04_Output/  (PDF, EPUB, DOCX)
       |
...
* **`output.docx`**: Formato Word per revisione o riutilizzo testuale.
